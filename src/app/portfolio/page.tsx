        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          ...
          <Card className="shadow-level1 border-0">
             <CardContent className="p-6">
               ...
               <div className="flex items-center mt-3">
                 <Badge className="bg-muted/10 text-muted-foreground border-muted-foreground/20 text-xs">
        ...
        <Tabs defaultValue="loans" className="w-full">
              ...
              <CardContent>
                <div className="space-y-4">
                  {portfolio.loans.filter(loan => loan.status === 'active').map((loan) => (
                     <div key={loan.id} className="p-6 border border-border rounded-xl hover:shadow-level1 transition-all duration-200 group">
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex items-center space-x-4">
                          <div className="w-12 h-12 bg-gradient-to-br from-agri-green to-sky-teal rounded-lg flex items-center justify-center">
                           ...
                           <div>
                             <h3 className="font-semibold text-foreground group-hover:text-agri-green transition-colors">
                               {loan.farmer}
                             </h3>
                             <p className="text-sm text-muted-foreground">{loan.location} • {loan.crop} Farming</p>
                           </div>
                        </div>
                      ...
                      {loan.status === 'active' && (
                        <div className="mb-4">
                           <div className="flex justify-between items-center mb-2">
                             ...
                             <span className="text-sm text-muted-foreground">{loan.paymentProgress}%</span>
                           </div>
                      ...
                      <div className="flex space-x-2">
                         ...
                         <Button variant="outline" size="sm" className="border-border hover:border-harvest-gold hover:text-harvest-gold">
              ...
              <CardContent>
                <div className="space-y-4">
                  {portfolio.loans.filter(loan => loan.status === 'repaid').map((loan) => (
                     <div key={loan.id} className="p-4 border border-border rounded-lg bg-muted/5">
                       <div className="flex items-center justify-between">
                         <div className="flex items-center space-x-3">
                           ...
                           <div>
                             <p className="font-medium text-foreground">{loan.farmer}</p>
                             <p className="text-sm text-muted-foreground">{loan.location}</p>
                           </div>
                         </div>
          ...
          <TabsContent value="risk" className="space-y-6">
                  ...
                  {portfolio.riskDistribution.map((risk, index) => (
                    <div key={index} className="space-y-2">
                       <div className="flex justify-between items-center">
                         <span className="font-medium text-foreground">{risk.risk} Risk</span>
                         <div className="flex items-center space-x-2">
                           <span className="text-sm text-muted-foreground">${risk.amount.toLocaleString()}</span>
                           <span className="text-sm font-medium text-foreground">{risk.percentage}%</span>
                         </div>
                       </div>
                       <div className="w-full bg-muted/10 rounded-full h-3">
                        <div
            ...
            <Card className="shadow-level1 border-0">
               ...
               <CardContent>
                 <div className="h-64 bg-muted/5 rounded-lg flex items-center justify-center">
                   <div className="text-center">
                     <PieChart className="w-12 h-12 text-muted-foreground/40 mx-auto mb-2" />
                     <p className="text-muted-foreground">Interactive risk heatmap coming soon</p>
                     <p className="text-sm text-muted-foreground/70">Visual representation of geographic risk distribution</p>
                   </div>
                 </div>
               </CardContent>
            </Card>
              ...
              <CardContent>
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {portfolio.loans.map((loan) => (
                     <div key={loan.id} className="border border-border rounded-xl overflow-hidden hover:shadow-level2 transition-all duration-200 group">
                       <div className="aspect-square bg-gradient-to-br from-agri-green/10 to-sky-teal/10 relative">
                         <div className="absolute inset-0 flex items-center justify-center">
                           <div className="text-center">
                             ...
                             <p className="text-sm font-medium text-foreground">{loan.farmer}</p>
                             <p className="text-xs text-muted-foreground">{loan.crop} Loan NFT</p>
                           </div>
                         </div>
                      ...
                      <div className="p-4">
                         <div className="space-y-2 mb-4">
                           <div className="flex justify-between text-sm">
                             ...
                             <span className="font-medium text-foreground">${loan.currentValue}</span>
                           </div>
                           <div className="flex justify-between text-sm">
                           ...
                           <div className="flex justify-between text-sm">
                             ...
                             <span className="font-medium text-foreground">{loan.aiScore}</span>
                           </div>
                         </div>

                         <div className="flex space-x-2">
                           <Button size="sm" variant="outline" className="flex-1 border-border hover:border-agri-green hover:text-agri-green">
